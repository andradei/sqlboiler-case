...
var (
	personColumns               = []string{"id", "name", "age"}
	personColumnsWithoutDefault = []string{"name", "age"}
...
func (o *Person) Things(mods ...qm.QueryMod) thingQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"persons_things\" on \"things\".\"id\" = \"persons_things\".\"thing_id\""),
		qm.Where("\"persons_things\".\"person_id\"=?", o.ID),
	)

	query := Things(queryMods...)
	queries.SetFrom(query.Query, "\"things\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"things\".*"})
	}
...
func (personL) LoadThings(ctx context.Context, e boil.ContextExecutor, singular bool, maybePerson interface{}, mods queries.Applicator) error {
	var slice []*Person
	var object *Person
	...
	query := NewQuery(
		qm.Select("\"things\".*, \"a\".\"person_id\""),
		qm.From("\"things\""),
		qm.InnerJoin("\"persons_things\" as \"a\" on \"things\".\"id\" = \"a\".\"thing_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Name, &one.BoughtOn, &localJoinCol)
		if err != nil {
...
func (o *Person) AddThings(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Thing) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"persons_things\" (\"person_id\", \"thing_id\") values ($1, $2)"
...
// Sets o.R.Persons's Things accordingly.
// Replaces o.R.Things with related.
// Sets related.R.Persons's Things accordingly.
func (o *Person) SetThings(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Thing) error {
	query := "delete from \"persons_things\" where \"person_id\" = $1"
	...
	removeThingsFromPersonsSlice(o, related)
	if o.R != nil {
		o.R.Things = nil
	}
...
func (o *Person) RemoveThings(ctx context.Context, exec boil.ContextExecutor, related ...*Thing) error {
	var err error
	query := fmt.Sprintf(
		"delete from \"persons_things\" where \"person_id\" = $1 and \"thing_id\" in (%s)",
...
func removeThingsFromPersonsSlice(o *Person, related []*Thing) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
